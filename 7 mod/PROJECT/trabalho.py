    ...
    if opçao1 == 1:
        ...
        if opcaologin == 1:
            ...
            for x in credenciais:  # for para percorrer o dicinario/ficheiro para entrar na conta e verificar se esta errada
                if decrypt_password(credenciais[x]) == senha and x == nome:  # entrar na conta
                    print("==================\nEntrou na conta com sucesso!\n==================")
                    login_valido = True
                    salvar_credenciais()
                    break
                elif decrypt_password(credenciais[x]) != senha and x == nome:  # erro caso senha esteja errada
        ...
        if opcaologin == 1:
            ...
            if len(entrada) != 2:
                ...
                for x in list(credenciais.keys()):  # Iterar sobre uma cópia das chaves
                    if decrypt_password(credenciais[x]) == senha and x == nome:
                        credenciais.pop(x)
                        salvar_credenciais()
